...
СХЕМА ВАЛИДИРОВАНА БОЕВЫМ ЗАКАЗОМ (16.06.2026, order=999_2853073_4bd502e0, 100 ₽):
касса приняла наш формат — HTTP 200 `Queued`, id=cd4ffc84…, ссылка на чек выдана.
Развилки, которых опасались, разрешены В ПОЛЬЗУ текущего кода: патент без НДС =
`Vat: null` (принят), `Quantity` числом 1 (принят), поля PascalCase `Vat`/`Label`
(приняты, не VatRate). Менять формат без причины НЕ нужно; при будущих 4xx начинать
разбор с этих трёх полей.
"""
