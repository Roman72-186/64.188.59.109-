...
# реальном отклонении заявки. `canceled` = заказ протух/брошен (Долями сам
# авто-отменяет неоплаченный заказ ~через 24ч) — это НЕ неудачная оплата
# (клиент просто не платил), рассылку «оплата не прошла» не запускаем.
