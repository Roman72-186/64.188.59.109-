...
- **09.06.2026 — `force` + `amount` в `/init-payment`** (commit `61e52d6`, 67 тестов).
  `force: true` — пропускает проверку активной ссылки (PRD §7.2), всегда создаёт новый
  платёж. Нужно для тестирования с разными суммами (`amount`). Задеплоено, сервис здоров.
...
**09.06.2026 — FORCE И AMOUNT ДЛЯ ТЕСТИРОВАНИЯ.** Поля `force: true` и `amount` в
`/init-payment` позволяют тестировать с разными суммами: `force` пропускает проверку
активной ссылки и всегда создаёт новый платёж (commit `61e52d6`, 67 тестов, задеплоено).
