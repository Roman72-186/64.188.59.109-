...
мультипровайдерный подход. TODO: ротировать боевые креды Долями у менеджера.
...
shalamov.io). Код на сервере (commit `ee3f60b`, 62 теста зелёные), `config.yaml` правлен
(`fail_tags_by_method`), сервис перезапущен и здоров. Осталось дождаться реального
негативного статуса в проде, чтобы увидеть `⛔ Отказ оплаты — тег отказа назначен` в логах —
и отдельно прежний открытый вопрос: позитив `commit → тег` (нужен платёж с прошедшим
скорингом BNPL). Детали — в разделе «Уточнения» выше.
