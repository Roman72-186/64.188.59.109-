...
- **02.07.2026 — ДОЛЯМИ НЕ ВЫДАВАЛ ССЫЛКУ: mTLS-СЕРТИФИКАТЫ НЕ ПЕРЕНЕСЛИ ПРИ
  МИГРАЦИИ — ИСПРАВЛЕНО. Попутно включены поллер Credit Broker и реконсилятор shalamo.**
  Жалоба: клиенту не пришла ссылка на оплату Долями. Разбор:
  1. **Причина:** заказ `314_3011564_c7fe3f6b` (контакт `3011564`, «Доступ в Сообщество
     Я ЕСТЬ», 17 000 ₽, `dolyami`, 06:31 UTC) упал на `create` с
     `[Errno 2] No such file or directory` — на новом сервере `72.56.8.174` не было
     файлов `2-certificate-2026-06-08-bnpl.pem` / `2-private1.key` (не перенесли при
     миграции 01.07; в конфиге пути были ещё и относительными). Карта/СБП работали.
     Полный разбор: `docs/incidents/2026-07-02-dolyame-mtls-certs-missing.md`.
  2. **Исправлено:** файлы скопированы из локального репо в `/opt/tbank_proxy/`
     (www-data, ключ 600), в `config.yaml` — абсолютные пути (бэкап
     `config.yaml.bak-20260702-dolyame-certs`), рестарт. mTLS проверен с сервера:
     `curl --cert --key -u ...  /v1/orders/<тест>/info` → 404 = хендшейк+Basic ok.
     **Клиенту нужно повторно нажать оплату Долями в боте** — ссылка теперь выдастся
     (сам перевыпуск через `/init-payment` не делался — создание боевого платежа
     агентом заблокировано разрешениями).
  3. **Поллер Credit Broker был выключен** (в `tbank_credit` не было
     `poll_interval_seconds`, дефолт 0) — а это единственный канал статуса кредитной
     заявки (webhookURL не передаётся). Включён `poll_interval_seconds: 60` (бэкап
     `config.yaml.bak-20260702-credit-poller`). Первый цикл успешно опросил висящую
     заявку `installment_10` того же контакта `3011564` (статус `new` — клиент не
     завершил оформление; заявка от 01.07, `f58eb473-...`).
  4. **Реконсилятор shalamo включён** (по просьбе владельца):
     `shalamo.reconcile_interval_seconds: 300` + `stranded_alert_after_seconds: 600`
     (бэкап `config.yaml.bak-20260702-reconciler`), рестарт, в логах
     «фоновый реконсилятор тегов каждые 300с». Теперь застрявшие «оплачено, но тег
     не назначен» добиваются автоматически (вчерашний случай `2893427` больше не
     потребует ручного разбора).
  5. Зависших «оплачено без тега» в БД нет (16 записей, все confirmed — с тегами).
     Тот же клиент `3011564` за сутки попробовал `installment_10` (18 350 ₽, new),
     `card_shop2` (16 900 ₽, pending) и `dolyami` (17 000 ₽, failed из-за п.1) —
     ни одна оплата пока не завершена.
  6. **Локальный `config.yaml` УСТАРЕЛ относительно серверного** (в нём ещё shalamo-auth
     через `?api_token=` и старый api_key; на сервере — рабочий вариант от 01.07).
     Источник истины — серверный `/opt/tbank_proxy/config.yaml`. Старый сервер
     `64.188.59.109` на 02.07 снова отвечает на ping (но выведен из эксплуатации).
     Урок миграции: переносить файлы вне git (сертификаты, config.yaml, .env, БД).

- **01.07.2026 (сессия 2) — ЖАЛОБА «НЕ ФОРМИРУЮТСЯ ССЫЛКИ НА ОПЛАТУ» — СЕРВЕР/КОД
  НЕ ВИНОВАТЫ, ПОДОЗРЕНИЕ НА НАСТРОЙКУ ЗАПРОСА В shalamov.io.**
  Владелец сообщил, что клиенты жалуются — в боте (shalamov.io) при попытке
  оплаты «ничего не происходит». Разбор:
  1. Сервер `72.56.8.174` жив и здоров (`/health` ok), DNS/nginx/TLS корректны.
  2. **Найден и исправлен реальный баг:** `tbank.terminal_password` на сервере
     отличался от локального `config.yaml` (разная длина!) — видимо испортился
     при «грязной» файловой миграции (см. запись про `app/schemas.py` выше).
     Исправлено точечно: бэкап `/opt/tbank_proxy/_backup/20260701-tbank-password/`,
     значение синхронизировано с локальным (сверено по SHA256-хэшу, сам пароль
     в чат не выводился), `systemctl restart tbank-proxy`. **Это не было
     причиной жалобы**, но был реальный самостоятельный баг — хорошо, что
     нашли и починили.
  3. **Основной боевой терминал по конфигу `1779970153075` (способы card/sbp)
     заблокирован Т-Банком** — `ErrorCode 202` в ответе Init. Уточнение по
     официальной документации Т-Банка (`developer.tbank.ru/eacq/intro/errors/
     error-codes`, через Context7): код **202 = «Терминал заблокирован»**,
     а не «неверные параметры» (текст `Message` в ответе — общая формулировка,
     не диагноз). Причина блокировки неизвестна (T-Bank/владелец должен
     смотреть в `business.tbank.ru` → Магазины/Терминалы, либо звонить в
     поддержку 8 800 700-66-66). Исправление пароля из п.2 эту ошибку НЕ
     убрало (значит блокировка — со стороны Т-Банка, не в конфиге).
  4. **НО владелец уточнил: этот терминал (1779970153075) фактически уже НЕ
     используется** — в проде реально работает только `card_shop2`/`sbp_shop2`
     (терминал `1780560752978`, реквизиты от Регины Габриелян 15.06.2026).
     Живым тестом подтверждено: `card_shop2` создаёт `pay_url` без ошибок
     (`payment_id=8774758278` и другие в логах 01.07 ~06:20–06:46 UTC).
     **Значит блокировка терминала 1779970153075 — НЕ причина жалобы клиентов**
     (просто сопутствующая находка, можно решить отдельно/не срочно).
  5. **Живая проверка через нагрузку.** Через `Monitor` слушали
     `/var/log/nginx/access.log` + `/opt/tbank_proxy/logs/app.log` в реальном
     времени. Первая попытка владельца (~06:39 UTC) НЕ оставила следа в логах
     сервера вообще — запрос не дошёл до сервера физически. Затем владелец
     стал тестировать сам через Postman (IP `54.86.50.139`, вероятно
     Postman Cloud/AWS) — первые попытки падали с `422` (тело `{"query":"str"}`
     — явно шаблон/плейсхолдер, потом `amount` как строка вместо числа), но
     **финальная попытка (06:46:16 UTC) прошла на 200** — `product=Доступ в
     Сообщество Я ЕСТЬ` (cart-режим, `community_yaest`), `method=card_shop2`,
     `amount=10000`, `force=true`, `has_email=true` → ссылка получена.
  6. **ВЫВОД СЕССИИ:** прокладка и терминал `card_shop2`/`sbp_shop2` полностью
     работоспособны end-to-end (подтверждено дважды живыми тестами). Проблема
     клиентов — вероятнее всего в том, **что/как отправляет автоматизация
     shalamov.io** при вызове `/init-payment`: либо запрос не долетает до
     `pay.sushi-house-39.ru` вообще (как в первой попытке владельца), либо
     shalamov.io шлёт похоже неполное/неверно типизированное тело (как ранние
     Postman-попытки — `amount` строкой, отсутствующие поля).
  7. **СЛЕДУЮЩИЙ ШАГ (не сделано в этой сессии):** открыть в личном кабинете
     shalamov.io настройку HTTP-запроса к прокладке (там, где вызывается
     `/init-payment` при попытке клиента оплатить) и сверить: (а) URL —
     должен быть ровно `https://pay.sushi-house-39.ru/init-payment` (не старый
     IP `64.188.59.109`, который сейчас полностью недоступен); (б) заголовок
     `X-Secret-Token` присутствует и верный; (в) тело — `contact_id`/
     `product_id`/`payment_method` строками, `amount` **числом** (в копейках),
     не строкой. Это находится вне репозитория — правится только в панели
     shalamov.io. Отдельно (не срочно, но зафиксировать): разобраться, почему
     Т-Банк заблокировал терминал `1779970153075` — раз он не используется,
     можно просто снять с баланса задач, но стоит понять причину блокировки
     на случай, если это симптом чего-то более широкого (например, аккаунт
     мерчанта Т-Банка под вопросом).
