...
```
Проект: прокладка оплат Т-Банк ↔ shalamo.io (Python 3.10+ · FastAPI · SQLite).
...
1. shalamo в config.yaml: сейчас заглушки (api_key=DEMO_SHALAMO_KEY, пути /contacts/tag и
   /contacts/variables — предполагаемые). Узнать у shalamo.io реальные: api_key, точные
...
сервера). Осталось: реальные доступы shalamo.io, домен, деплой на VPS, тестовый платёж.
