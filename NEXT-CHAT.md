...
**16.06.2026 (сессия 3, доп.) — РАССРОЧКУ/КРЕДИТ ТОЖЕ ФИСКАЛИЗИРУЕМ ЧЕРЕЗ CloudKassir
(ЗАДЕПЛОЕНО, эффекта пока нет).** Решение владельца: «касса по аналогии с Долями» —
т.е. чек по рассрочке/кредиту пробивает прокладка через CloudKassir, а НЕ автофискализация
Т-Банка. Изменение: `fiscalize_providers: [dolyame] → [dolyame, tbank_credit]` (config.yaml
локально + сервер, бэкап `config.yaml.bak-20260616-precredit-fisc`; CLAUDE.md и
config.example.yaml обновлены, коммит `bb230e9`). Кода НЕ требует — механизм общий
(build_customer_receipt + фоновая реконсиляция). После рестарта в логах:
`CloudKassir: фоновая фискализация (каналы: dolyami, installment_3/6/10)`. Проверено:
нефискализированных кредитных заказов нет (`[]`), сюрприз-чеков по старым заказам нет
(бэкфилл защитил). **Эффекта пока ноль** — кредитных заказов не бывает (курс 99 ₽ <
минимума рассрочки 3108 ₽).
- **ДВА УСЛОВИЯ ДО ПЕРВОГО РЕАЛЬНОГО КРЕДИТНОГО ЗАКАЗА (≥3108 ₽):**
  1. ✅ **ЗАКРЫТО (владелец подтвердил 16.06): касса на стороне банка отключена** → Т-Банк
     по POS-кредитованию чек сам НЕ пробивает, двойного чека не будет, мы единственный
     фискализатор рассрочки/кредита. (card/СБП — отдельная касса в эквайринговом терминале,
     не задета: 309-ошибок в Init нет. Финально убедиться разовой картой — чек покупателю.)
  2. Признак расчёта кредита: сейчас как у Долями (`payment_method: full_prepayment` →
     Method 1, бакет `Amounts.Electronic`). Если бухгалтер потребует «передача в кредит»
     (Method 7) и `Amounts.Credit` — нужен per-провайдер override в `build_customer_receipt`
     (`app/cloudkassir.py`; сейчас шлёт общий блок `receipt`). Для патента без НДС, скорее
     всего, текущего варианта достаточно (как у Долями — валидировано).
