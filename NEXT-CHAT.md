...
- **08.06.2026 — ТЕГ ОТКАЗА (реализован + протестирован, 62 теста; НЕ активирован в проде).**
  При терминальном негативе Долями (`rejected`/`canceled`) прокладка теперь назначает
  отдельный «тег отказа» — триггер авторассылки «оплата не прошла». Успех (`paid_dolyami_*`)
  не затронут. Best-effort: сбой shalamo логируется, webhook отвечает `OK` (нет `paid_at` →
  ...
  **Чтобы включить в проде (3 шага):**
  1. Создать тег `fail_dolyami_basic` (имя на твой выбор) в shalamov.io и навесить на него
     авторассылку «оплата не прошла».
  2. В серверном `config.yaml` под `products.course_basic` (после `tags_by_method`, ~стр.100)
     добавить:
     ```yaml
     fail_tags_by_method:
       dolyami: fail_dolyami_basic
     ```
  3. Задеплоить новый код + рестарт сервиса. Миграция БД (`ALTER TABLE ADD COLUMN
     fail_tag_assigned_at`) выполняется автоматически на старте, идемпотентна, данные целы.
  Проверка в проде: негативный скоринг Долями → в логах `⛔ Отказ оплаты — тег отказа назначен`.
