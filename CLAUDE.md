...
- **CloudKassir — единая онлайн-касса (фискализация 54-ФЗ)** ([app/cloudkassir.py](app/cloudkassir.py)):
  ...
  `fiscalization: disabled` (иначе двойной чек). **СТАТУС: ВАЛИДИРОВАНО боевым заказом
  Долями** (16.06, `/kkt/receipt` → HTTP 200 Queued, чек ушёл). Боевой
  `fiscalize_providers: [dolyame, tbank_credit]` — рассрочку/кредит решено фискализировать
  тоже через CloudKassir (по аналогии с Долями, НЕ автофискализация Т-Банка). **Два условия
  до первого реального кредитного заказа** (≥3108 ₽; сейчас их нет — курс 99 ₽): (1) Т-Банк
  по POS-кредитованию не должен фискализировать сам (иначе двойной чек); (2) признак расчёта
  кредита сейчас как у Долями (`full_prepayment`/`Amounts.Electronic`) — если бухгалтер
  потребует «передача в кредит»/`Amounts.Credit`, нужен per-провайдер override в
  `build_customer_receipt` (сейчас шлёт общий блок `receipt`). Карту/СБП (tbank) НЕ включаем.
