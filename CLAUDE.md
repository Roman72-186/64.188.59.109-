...
Прокладка (Python 3.10+ · FastAPI · SQLite) между Т-Банком и shalamo.io: принимает
оплату через Т-Банк и после подтверждённого платежа назначает контакту тег в
shalamo.io (тег запускает авторассылку). Касса/чеки/возвраты — вне scope.
**[PRD.md](PRD.md)** — источник истины по требованиям; **[DOCS.md](DOCS.md)** —
установка/настройка/эксплуатация (синхронизирован с кодом).

Поток: бот → `POST /init-payment` → Т-Банк выдаёт `pay_url` → пользователь платит →
`POST /webhook/tbank` → синхронно назначается тег в shalamo.io → авторассылка по тегу.
Тег **только назначается** прокладкой; удаляется первым шагом авторассылки внутри shalamo.io.
...
venv\Scripts\python -m pytest -q                    # 23 теста: unit + интеграционные
